...
//		this->debug_register_set = false;
	...
	if (this->lcdEnabled())
	{
		...
		switch (this->getMemory(STAT) & 0b00000011)
		{
		case 0: // h blank
		{
...
Word PPU::getTileAddressFromNumber(const Byte tile_number, const enum eTileType eTileType)
{
	...
	case PPU::window:
	{
		bool addressing_mode = (this->getMemory(LCDC) & (0b1 << 4));
...
{
	//Byte* lcdstat_register_ptr = &this->bus->io[STAT - IOOFFSET];
	Byte original_mode = (this->getMemory(STAT) & 0x3);
	bool irq_needed = false;
	...
	case 0: {
		...
		irq_needed = (this->getMemory(STAT) & 0b00001000);
	}break;
	...
	case 1: {
		...
		if (this->lcdEnabled())
			irq_needed = (this->getMemory(STAT) & 0b00010000);
	}break;

	case 2: {
		//*lcdstat_register_ptr = ((*lcdstat_register_ptr & 0b11111100) | 0b00000010);
		this->setMemory(STAT, ((this->getMemory(STAT) & 0xFC) | 0x2));
		irq_needed = (this->getMemory(STAT) & 0b00100000);
	}break;
	...
	if ((original_mode != (this->getMemory(STAT) & 0x3)) && irq_needed)
...
{
	return this->getMemory(STAT) & 0b00000011;
}
